...
function EditProfilePopup({ isOpen, onClose, onUpdateUser, isLoading }) {
   ...
   return (
      <PopupWithForm name='editProfile' title='Редактировать профиль' buttonText={buttonText} isOpen={isOpen}
                     onClose={onClose} onSubmit={handleSubmit}
                     isFormValid={formValid}> {/* попап редактирования профиля */}
         <label htmlFor="inputName">
            <input onChange={name.handleChange} value={name.value || ''} type="text" name="name"
                   className={inputNameClassName} autoComplete="off"
         ...
         <label htmlFor="inputJob">
            <input onChange={description.handleChange} value={description.value || ''} type="text" name="about"
                   className={inputDescriptionClassName} autoComplete="off"
