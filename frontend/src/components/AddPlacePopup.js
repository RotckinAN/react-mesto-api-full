...
function AddPlacePopup({ isOpen, onClose, onAddPlaceSubmit, isLoading }) {
   ...
   return (
      <PopupWithForm name='addPicture' title='Новое место' buttonText={buttonText} isOpen={isOpen} onClose={onClose}
                     onSubmit={handleSubmit} isFormValid={formValid}> {/* попап добавления новых фото */}
         <label htmlFor="inputPicturePopup">
            <input onChange={name.handleChange} type="text" name="name" className={inputNameClassName}
                   autoComplete="off" id="inputPicturePopup" value={name.value} placeholder='Название' minLength="2"
                   maxLength="30" required/>
            <span className={inputNameMessageErrorClassName}>{name.inputError}</span>
         </label>
         <label htmlFor="inputPictureLink">
            <input onChange={link.handleChange} type="url" name="link" className={inputLinkClassName} autoComplete="off"
                   id="inputPictureLink" value={link.value} placeholder='Ссылка на картинку' required/>
